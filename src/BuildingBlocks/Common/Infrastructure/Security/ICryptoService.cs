...
public interface ICryptoService
{
    ...
    /// <param name="plainText">암호화할 평문</param>
    /// <returns>Base64 인코딩된 암호화 문자열</returns>
    string EncryptWithNoVector(string plainText);
    ...
    /// <param name="encryptedText">Base64 인코딩된 암호화 문자열</param>
    /// <returns>복호화된 평문</returns>
    string DecryptWithNoVector(string encryptedText);

    /// <summary>
    /// 지정된 키 타입으로 AES-256 암호화된 문자열을 암호화 [Vector 값은 Zero]
    /// </summary>
    /// <param name="plainText">암호화할 평문</param>
    /// <param name="keyType">사용할 키 타입</param>
    /// <returns>복호화된 평문</returns>
    string EncryptWithNoVector(string plainText, CryptoKeyType keyType);
