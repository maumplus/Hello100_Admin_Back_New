...
using Hello100Admin.Modules.Admin.Infrastructure.Persistence.DbModels.Member;
...
namespace Hello100Admin.Modules.Admin.Infrastructure.Repositories.Member
{
    public class MemberStore : IMemberStore
    {
        ...
        public MemberStore(IDbConnectionFactory connectionFactory, ILogger<MemberStore> logger)
        {
        ...
        public async Task<MemberEntity?> GetByIdAsync(
            string uid,
            CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                var dbMember = await connection.QueryFirstOrDefaultAsync<MemberRow>(sql, new { Uid = uid });
                if (dbMember == null)
                {
        ...
        public async Task<MemberEntity?> GetMember(string uid, CancellationToken cancellationToken = default)
        {
            try
            {
        ...
        public async Task<IEnumerable<MemberFamilyEntity?>> GetMemberFamilys(string uid, CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                WHERE uid = @Uid
                    AND del_yn='N'
            ";
                var dbMemberFamilys = await connection.QueryAsync<MemberFamilyRow>(sql, new { Uid = uid });
                if (dbMemberFamilys == null)
                {
                    _logger.LogWarning("No Member families found for Uid: {Uid}", uid);
                    return Enumerable.Empty<MemberFamilyEntity?>();
                }
        ...
        // DB 모델 → 도메인 모델 매핑
        private MemberEntity MapToDomain(MemberRow db)
        {
        ...
        private MemberFamilyEntity MapToDomain(MemberFamilyRow db)
        {
